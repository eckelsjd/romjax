...
class ConstantCoordinateVelocity(ForcingCallable):
    ...
    def callable(self, inputs: Inputs, outputs: AdvectionDiffusionOutputs) -> jax.Array:
        ...
        vx_x = jnp.asarray(inputs['vx_x'])
        vx_y = jnp.asarray(inputs['vx_y'])
        vy_x = jnp.asarray(inputs['vy_x'])
        vy_y = jnp.asarray(inputs['vy_y'])
...
class PotentialVelocity(ForcingCallable):
    ...
    :ivar potential: forcing callable used to generate the scalar streamfunction field
    """

    normalize: bool = False
    potential: Annotated[ForcingCallable, BeforeValidator(functools.partial(from_registry, _forcing_registry))] = (
        Field(default_factory=ConstantForcing)
    )

    class Inputs(DictModel):
        """Inputs for the streamfunction velocity forcing.

        The full input mapping is forwarded to ``potential``.  Thus a constant
        streamfunction can be supplied with ``const`` or a spatial forcing can
        consume ``coords`` and its own coefficients.
        """
    ...
    def callable(self, inputs: Inputs, outputs: AdvectionDiffusionOutputs) -> jax.Array:
        """Evaluate the components-first velocity field generated by ``potential``.

        :param inputs: potential forcing inputs and solver coordinates
        :param outputs: current transport outputs, unused
        :return: velocity with shape ``(2, nx, ny)``
        """
        psi = jnp.asarray(self.potential(inputs, outputs))
        if psi.ndim != 2:
...
_forcing_registry["potential"] = PotentialVelocity
