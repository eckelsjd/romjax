...
def test_poisson_defaults_allow_generic_inputs() -> None:
    model = Poisson2D(
        forcing="gaussian",
        forcing_defaults={"custom": 1.0},
    ...
    assert model.forcing_defaults == {"custom": 1.0}
    assert model.conductivity_defaults == {"alpha": 0.25}
